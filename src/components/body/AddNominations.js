	...
	onSubmit = () => {
		OMDB.get(``, { params: { s: this.state.in, type: 'movie' } }).then(({ data }) => {
			if (data.Response === 'True') {
				this.setState({ result: data.Search, page: 1, total: data.totalResults, term: this.state.in });
			...
			return (
				<Fragment>
					<div className="adno-divider">
						<div className="adno-divider__prev" role="button" onClick={() => this.changePage(-1)}>
							<i class="arrow circle left icon" style={{ margin: '0 8px' }} />
							Previous
						</div>
						<div className="adno-divider__page">Page {this.state.page}</div>
						<div className="adno-divider__next" role="button" onClick={() => this.changePage(1)}>
							Next
							<i class="arrow circle right icon" style={{ margin: '0 8px' }} />
						</div>
					</div>
					<div className="movie-con">{this.state.result.map((d) => <Movie data={d} />)}</div>
				</Fragment>
			);
		}
	}
